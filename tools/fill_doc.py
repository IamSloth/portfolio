...
def set_cell(table, row, col, value):
    """셀 텍스트 교체 (서식 보존). 0-indexed grid 좌표."""
    try:
...
def fill_resume(doc, profile, project_root):
    ...
    # 응시파트
    set_cell(t, 2, 18, 'ESG 사무')

    # 인적사항
    set_cell(t, 4, 5, p['name_ko'])
    set_cell(t, 4, 15, p['birthdate'].replace('-', '.'))
    set_cell(t, 5, 5, p['email'])
    set_cell(t, 5, 15, p['phone'])
    set_cell(t, 6, 5, p['address'])
    ...
    if hs:
        ...
        if len(period) == 2:
            set_cell(t, 9, 1, period[0])
            set_cell(t, 9, 6, period[1])
        set_cell(t, 9, 8, hs['school'].replace('고등학교', ''))
        set_cell(t, 9, 19, '졸업')
    ...
    if undergrad:
        ...
        if len(period) == 2:
            set_cell(t, 10, 1, period[0])
            set_cell(t, 10, 6, period[1])
        set_cell(t, 10, 8, '숭실')
        set_cell(t, 10, 13, '전기공학/경제학(복수)')
        set_cell(t, 10, 19, '졸업')
        set_cell(t, 10, 22, undergrad.get('gpa', ''))
    ...
    if grad:
        ...
        if len(period) == 2:
            set_cell(t, 11, 1, period[0])
            set_cell(t, 11, 6, period[1])
        set_cell(t, 11, 8, '숭실 사회복지')
        set_cell(t, 11, 13, '사회적기업')
        set_cell(t, 11, 19, '졸업')
        set_cell(t, 11, 22, grad.get('gpa', ''))
    ...
    for i, (start, end, company, role, salary, reason) in enumerate(career_data):
        row = 14 + i
        set_cell(t, row, 1, start)
        set_cell(t, row, 6, end)
        set_cell(t, row, 8, company)
        set_cell(t, row, 12, role)
        set_cell(t, row, 17, salary)
        set_cell(t, row, 21, reason)

    # PC 활용능력 (R20~R23)
    set_cell(t, 20, 4, '상')
    set_cell(t, 21, 4, '상')
    set_cell(t, 22, 4, '상')
    set_cell(t, 23, 4, 'SQL, AI Tools')

    # 자격증
    certs = [('사회복지사 1급', '2020.02'), ('1종대형 운전면허', '2019.02')]
    for i, (name, date) in enumerate(certs):
        set_cell(t, 20 + i, 7, name)
        set_cell(t, 20 + i, 9, date)
